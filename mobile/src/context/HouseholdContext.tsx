...
export function HouseholdProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function loadHouseholds() {
      try {
        ...
        if (data.length > 0) {
          const savedHouseholdId = await AsyncStorage.getItem("current_household_id");
  ...
  async function setCurrentHousehold(household: Household) {
    setCurrentHouseholdState(household);
    await AsyncStorage.setItem("current_household_id", String(household.id));
  }
